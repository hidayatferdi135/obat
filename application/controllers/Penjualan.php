...
class Penjualan extends CI_Controller {   
    ...
    public function ajaxPaginationDataKasir($kode)
    {
        ...
        $kategori = $kode <= 2? 'OOT' : ($kode == 3? 'OOT' : ($kode == 4? 'Prekursor' : ''));
        // $kategori = 'Prekursor';
        // $kategori1 = 'Prekursor';
        ...
        // if ($kode <= 2) {
        //     $totalRec = count($data['post']);
        //     $data['loop'] = $data['post'];
        // }else if ($kode == 3 ) {
        //     $totalRec = count($data['post1']);
        //     $data['loop'] = $data['post1'];
        // }else if($kode == 4) {
        //     $totalRec = count($data['post1']);
        //     $data['loop'] = $data['post1'];
        // }

        // echo json_encode(['Data' => $data['loop'],'Kategori' => $kategori]);die;
    ...
    {    
        ...
        // $kategori = 'Prekursor';
        // $kategori1 = 'Prekursor';
        $data['post1'] = $this->penjualan_model->getRows1($kategori);
        
        //get the posts data
        $data['post'] = $this->penjualan_model->getRows();
        // echo json_encode($data['post1']);die;
        // $data['post2'] = $this->penjualan_model->getRows2(array($where1, $kategori, 'limit'=>$this->perPage));
        $data['dokter'] = $this->db->get('master_dokter')->result();
        $data['spg'] = $this->penjualan_model->get_spg();
    ...
    {   
        ...
        foreach($query->result() as $r) { 
            $now = date("Y-m-d");
            $masa = date("Y-m-d", strtotime($r->tgl_masa));
            $tmasa = date('Y-m-d', strtotime('+6 month', strtotime($masa)));
            if ($tmasa < $now) {
            ...
            $this->security->xss_clean($r->apoteker), 
            $this->security->xss_clean($r->telepon),
            '    <a onclick="pilihpembeli1(this)" data-tgl_masa="'.$r->tgl_masa.'" data-namapembeli="'.$r->nama_pembeli.'" data-id="'.$r->id.'" class="mt-xs mr-xs btn btn-info" role="button"><i class="fa fa-check-square-o"></i></a>
            '
            ); 
            }
	...
	{    
        ...
        $data['tempo'] = $this->input->post('tanggal_jatuh_tempo');
	...
	{    
        ...
        $data['status'] =  "Kredit";
        $data['kepada'] =  "Costumer Toko";
        $tempo = date("d M Y", strtotime($this->input->post('tanggal_jatuh_tempo')));
         $data['spg'] = $this->input->post('nama_spg');
         $data['tempo'] = $tempo;

         // $this->load->view('member/penjualan/struk', $data);
        $html = $this->load->view('member/penjualan/struk', $data);
        $html2 = $this->output->get_output($html);
        $this->load->library('pdf');
        $this->dompdf->loadHtml($html2);
        $this->dompdf->setPaper('A4', 'portrait');
        $this->dompdf->render();
        $this->dompdf->stream('Struk_kredit', array('Attachment'=>0));
    }
}